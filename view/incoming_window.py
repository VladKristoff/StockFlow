...
from database.db_doc_manager import get_all_documents, get_document_items, get_document_by_id, delete_document, create_document
from database.db_manager import get_any_table
import os
from datetime import datetime

from utils.doc_generator import create_incoming_invoice
...
class IncomingWindow:
    def __init__(self, main_frame, app):
        ...
        self.product_map = {row[1]: (row[0], row[2]) for row in self.products}  # name -> (id, price)
    ...
    def open_add_dialog(self):
        AddDocumentDialog(self.main_frame, self, doc_type=1)
    ...
    def view_document(self, event):
        ...
        if selection:
            ...
            ViewDocumentDialog(self.main_frame, self, doc_id, doc_type=1)
    ...
    def delete_document(self):
        ...
        if messagebox.askyesno("Подтверждение", "Удалить выбранный документ? Это действие нельзя отменить!"):
            ...
            success, message = delete_document(doc_id, 1)
            if success:
...
class AddDocumentDialog:
    def __init__(self, parent, window, doc_type=1):
        self.window = window
        self.doc_type = doc_type
        self.items = []

        self.dialog = Toplevel(parent)
        self.dialog.title("Добавление документа")
    ...
    def create_widgets(self):
        ...
        self.contractor_combo.grid(row=0, column=1, sticky="w", pady=5, padx=10)
        ...
        self.employee_combo.grid(row=1, column=1, sticky="w", pady=5, padx=10)
        ...
        self.date_entry.grid(row=2, column=1, sticky="w", pady=5, padx=10)
        ...
        button_frame = Frame(main_frame, background="#3C4250")
        button_frame.grid(row=7, column=0, columnspan=2, pady=20)

        ttk.Button(button_frame, text="Сохранить", command=self.save_document, style="Button.TButton",
               width=15).pack(side="left", padx=10)
        ttk.Button(button_frame, text="Отмена", command=self.dialog.destroy, style="Button.TButton",
               width=15).pack(side="left", padx=10)

    def add_product(self):
        AddProductDialog(self.dialog, self)
    ...
    def update_items_table(self):
        ...
        total = 0
        for product_name, product_id, quantity, price in self.items:
            summa = quantity * price
            total += summa
            self.items_tree.insert("", "end", values=(product_name, quantity, f"{price:.2f}", f"{summa:.2f}"))
    ...
    def save_document(self):
        ...
        # Подготовка товаров
        items_for_db = [(prod_id, qty, price) for _, prod_id, qty, price in self.items]

        from database.db_doc_manager import create_document
        success, message, doc_id = create_document(self.doc_type, doc_date, contractor_id, employee_id, items_for_db)
        ...
        if success:
            ...
            if messagebox.askyesno("Создать документ", "Создать Word документ накладной?"):
                ...
                items_for_word = [{'product_name': name, 'quantity': qty, 'price': price}
                                  for name, _, qty, price in self.items]

                filepath = create_incoming_invoice(doc_id, doc_data, items_for_word)
...
class AddProductDialog:
    ...
    def create_widgets(self):
        ...
        ttk.Label(main_frame, text="Товар:", style="SmallText.TLabel", font="Arial, 14").grid(row=0, column=0, sticky="w",
                                                                                            pady=5)

        self.product_combo = ttk.Combobox(main_frame, values=list(self.parent_dialog.window.product_map.keys()),
                                          width=30)
        self.product_combo.grid(row=0, column=1, sticky="w", pady=5, padx=10)

        ttk.Label(main_frame, text="Количество:", style="SmallText.TLabel", font="Arial, 14").grid(row=1, column=0,
                                                                                                 sticky="w", pady=5)
        self.quantity_entry = Entry(main_frame, width=20)
        self.quantity_entry.grid(row=1, column=1, sticky="w", pady=5, padx=10)

        ttk.Label(main_frame, text="Цена:", style="SmallText.TLabel", font="Arial, 14").grid(row=2, column=0, sticky="w",
                                                                                           pady=5)

        self.price_label = Label(main_frame, text="0.00 руб.", bg="#3C4250", fg="#B9FFDC", font=("Arial", 12))
        self.price_label.grid(row=2, column=1, sticky="w", pady=5, padx=10)
...
class ViewDocumentDialog:
    def __init__(self, parent, window, doc_id, doc_type=1):
        self.window = window
        self.doc_id = doc_id
        self.doc_type = doc_type
    ...
    def print_document(doc_data, items):
        ...
        filepath = create_incoming_invoice(doc_data[0], doc_info, items_for_word)
        messagebox.showinfo("Успех", f"Документ сохранен: {filepath}")
        os.startfile(filepath)